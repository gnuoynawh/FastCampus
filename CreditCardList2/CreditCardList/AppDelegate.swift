...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        db.collection("creditCardList").getDocuments { snapshot, _ in
            ...
            do {
                try batch.setData(CreditCardDummyModify.card0, forDocument: card0Ref)
                try batch.setData(CreditCardDummyModify.card1, forDocument: card1Ref)
                try batch.setData(CreditCardDummyModify.card2, forDocument: card2Ref)
                try batch.setData(CreditCardDummyModify.card3, forDocument: card3Ref)
                try batch.setData(CreditCardDummyModify.card4, forDocument: card4Ref)
                try batch.setData(CreditCardDummyModify.card5, forDocument: card5Ref)
                try batch.setData(CreditCardDummyModify.card6, forDocument: card6Ref)
                try batch.setData(CreditCardDummyModify.card7, forDocument: card7Ref)
